...
fn parse_source(source: &[u8], config: &DiagnosticConfig) -> Vec<mib_rs::ast::Module> {
    let mut sources = SourceSet::new();
    let id = sources
        .insert(
            SourceOrigin::memory("parser-corpus"),
            "parser-corpus",
            Arc::from(source),
        )
        .unwrap();
    mib_rs::parser::parse(sources.get(id).unwrap(), config)
}

fn parse_file(path: &Path) -> Vec<mib_rs::ast::Module> {
    let content = std::fs::read(path).unwrap();
    parse_source(&content, &DiagnosticConfig::default())
}
...
END
"#;
    let modules = parse_source(input, &DiagnosticConfig::default());
    assert_eq!(modules.len(), 1);
    assert!(parse_errors(&modules).is_empty());
